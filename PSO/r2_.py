...
#         ch_names.append(ch_list[same_index[i]])
    ...
    raw_hlfilter = raw.filter(l_freq=15, h_freq=100,method='fir')
...
def true_data():
    ...
    # 指定CNT文件路径
    cnt_file = '/public/home/ynhang/yuze/code/test/data/A_MI.cnt'

    # 使用mne加载CNT文件
    raw = mne.io.read_raw_cnt(cnt_file, preload=True)

    # 0.1Hz~100Hz高低通滤波，以及50Hz陷波滤波
    raw_hlfilter = raw.copy().filter(l_freq=15, h_freq=100,method='fir')
...
def r2_cal(data):
    ...
    r2 = r2_score(data_true[0,0:100], np.log10(S[0,0:100]))
    
    sum = 0
    for i in range(len(data_true)):
        t = r2_score(data_true[i,15:100], np.log10(S[i, 15:100]))
        sum = sum + t
